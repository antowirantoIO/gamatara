...
class BodController extends Controller
{
        ...
        try{
            ...
            foreach ($data as $item) {
                ...
                if ($item->complaint->isEmpty()) {
                    $status = 1;
                } elseif ($item->complaint->where('id_bod_approval', null)->isNotEmpty()) {
                    $status = 2;
    ...
    {
        try{
            $data = OnRequest::with(['complaint','complaint.vendors:id,name',
                                'customer:id,name','pm.karyawan:id,name,nomor_telpon',
                                'pm.pas.karyawan:id,name,nomor_telpon',
                                'pm.pes.karyawan:id,name,nomor_telpon',
